...
class TradeScenario(TinkoffBrokerServer):
    ...
    def _UpdateOrderBook(self) -> bool:
        while True:
            # Очистка файла example.txt
            with open('example.txt', 'w') as clear_file:
        ...
        if self._ordersBook["buy"] and self._ordersBook["sell"]:
            ...
            while True:
                ...
                new_data = self._curPriceToBuy

                # Запись новых данных в example.txt построчно
                with open('example.txt', 'a') as new_data_file:
                    new_data_file.write(str(new_data) + '\n')

                # Чтение старых данных из example1.txt
                with open('example1.txt', 'r') as old_data_file:
                    old_data1 = old_data_file.readlines()

                # Чтение тикеров из tickers.txt
                with open('tickers.txt', 'r') as tickers_file:
                    tickers = tickers_file.readlines()

                # Сравнение новых данных с предыдущими данными
                for i, (old_line, new_line, ticker) in enumerate(zip(old_data1, new_data_file, tickers)):
